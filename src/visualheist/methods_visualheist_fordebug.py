...
def _save_image_from_bbox(image, annotation, output_dir, pdf_name, page_num, blur_level):
    """
    Save cropped figure/table images from the image based on bounding boxes.
    Uses page and bbox number in the filename to avoid collisions.
    """
    output_dir = Path(output_dir)
    for i, bbox in enumerate(annotation['bboxes']):
        x1, y1, x2, y2 = bbox
        cropped = image.crop((x1, y1, x2, y2))
        filename = f"{pdf_name}_page{page_num}_blur{blur_level}_bbox{i + 1}.png"
...
def _pdf_to_figures_and_tables(pdf_path, output_dir, large_model):
    ...
    for i, image in enumerate(images):
        blurred_versions = generate_blurred_versions(image, num_levels=4)
        for blur_level, blurred_image in enumerate(blurred_versions, start=1):
            annotation = _tf_id_detection(blurred_image, model, processor)
            tag = f"{pdf_name}_blur{blur_level}"
            _save_image_from_bbox(blurred_image, annotation, output_dir, pdf_name, page_num=i, blur_level=blur_level)
            print(f"Page {i}, Blur {blur_level} saved. Objects: {len(annotation['bboxes'])}")
...
def generate_blurred_versions(image, num_levels=4):
    """
    Generate blurred versions of an image using increasing Gaussian blur.
    Returns a list: [blur1, blur2, ..., blurN]
    """
    return [image.filter(ImageFilter.GaussianBlur(radius=i)) for i in range(1, num_levels + 1)]
...
def batch_pdf_to_figures_and_tables(input_dir, output_dir=None, large_model=False):
    ...
    print("Organizing into blur levels")
    for file in output_dir.glob("*.png"):
        parts = file.stem.split("_")
        for part in parts:
            if part.startswith("blur"):
                blur_level = part
                target_dir = output_dir / blur_level
